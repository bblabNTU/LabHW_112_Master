...
//    cv::dnn::blobFromImage(netInputImg, blob, 1 / 255.0, cv::Size(netWidth, netHeight), cv::Scalar(114, 114,114), true, false);
    net.setInput(blob);
    std::vector<cv::Mat> netOutputImg;
    net.forward(netOutputImg, net.getUnconnectedOutLayersNames());  // !!!vector subscript out of range!!!
#if CV_VERSION_MAJOR==4&&CV_VERSION_MINOR==7
    std::sort(netOutputImg.begin(), netOutputImg.end(), [](cv::Mat &A, cv::Mat &B) {return A.size[2] > B.size[2]; });//opencv 4.6
#endif
    ...
    // for迴圈 crashed
    float* pdata = (float*)netOutputImg[0].data;  //new
...
//                    std::cout<<"box_score: "<<box_score<<std::endl;
                    if (box_score >= boxThreshold) {
                        ...
                        max_class_socre = (float)max_class_socre; // sigmoid_x(max_class_socre)  (float)max_class_socre
...
void Yolo::drawPred_save(cv::Mat& img, std::vector<Output> result, std::vector<cv::Scalar> color, std::string save_path) {
    for (int i = 0; i < result.size(); i++) {
        int left, top;
        left = result[i].box.x;
        top = result[i].box.y;
        ...
        putText(img, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 1, color[result[i].id], 2);
        //save cropping image
        cv::Rect roi(left, top, result[i].box.width, result[i].box.height);
        cv::Mat croppedImage = img(roi).clone();
        cv::imwrite(save_path, croppedImage);
    }
